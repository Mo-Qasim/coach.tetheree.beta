...
const Index = () => {
    ...
    return (
        <>
            <nav className="bg-transparent py-20 lg:py-28">
                <div className="container mx-auto px-20 md:px-24 lg:px-32">
                    <div className="flex items-center justify-between">
                        <img className='h-36 md:h-50' src="/assets/svg/logo-nav.svg" alt="" />
                        <ul className="hidden lg:flex items-center gap-30">
                            {['Product', 'Use Case', 'Workflow', 'Pricing']?.map((item, _index) => (
                                <li key={_index} className='text-sm text-white font-normal transition-all cursor-pointer hover:underline hover:opacity-90'>{item}</li>
                            ))}
                        </ul>
                        <div className="flex items-center">
                            <span className='hidden lg:block text-sm text-white font-normal transition-all cursor-pointer hover:underline hover:opacity-90'>Sign in</span>
